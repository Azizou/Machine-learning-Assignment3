...
{
	if (m_iClosestMine >= 0 && m_iClosestMine < objects.size()){
	...
	if (m_iClosestRock >= 0 && m_iClosestRock < objects.size()){
		SVector2D<double> DistToObject = m_vPosition - objects[m_iClosestRock]->getPosition();
	...
	if (m_iClosestSupermine >= 0 && m_iClosestSupermine < objects.size()){
		SVector2D<double> DistToObject = m_vPosition - objects[m_iClosestSupermine]->getPosition();
