...
int CDiscMinesweeper::CheckForObject(vector<CDiscCollisionObject*> &objects, int size)
{
	...
	if (m_iClosestRock >= 0 && m_iClosestRock < objects.size()){
		SVector2D<int> DistToObject = m_vPosition - objects[m_iClosestRock]->getPosition();
	...
	if (m_iClosestSupermine >= 0 && m_iClosestSupermine < objects.size()){
		SVector2D<int> DistToObject = m_vPosition - objects[m_iClosestSupermine]->getPosition();
